...
export default class View {
    ...
    getStudentId(el) {
        console.log(el.closest(View.STUDENTS_ITEM_SELECTOR))
        return el.closest(View.STUDENTS_ITEM_SELECTOR)?.dataset.id;
    }

    generateHtmlList(item){
        return ` 
       <li class="${style.students_container__item} students_container__item" data-id="${item.id}">
            <strong class="${style.students_container__name}">${item.name}</strong>
            ${this.createInputs(item.marks)}
            <button class="${style.button} button_delete">
    ...
    createInputs(marks){
        ...
        for (let i = 0; i < marks.length; i++){
            result += `<input type="text" class="${style.students_container__marks} students_container__marks" value="${marks[i]}">`
        }
